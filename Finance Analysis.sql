...
SELECT B.BranchName, A.BranchID, round(AVG(A.Balance), 2) as avg_account_balance
...
select C.CustomerID, concat(C.FirstName, ' ' ,LastName) as customer_name, sum(A.balance) as current_balance
...
CONCAT(C.firstname, ' ', C.lastname) AS Name,
 COUNT(T.AccountID) AS most_transactions
...
SELECT 
    Name, most_transactions FROM
    most_t where top = 1;
    
---- Q8. Which branch has the highest total balance across all of its accounts? ---
select A.AccountId, B.BranchName, sum(A.Balance) as total_balance  
...
select  C.CustomerID, concat(C.FirstName, ' ' ,C.LastName) as customer_name, sum(A.Balance) as total_balance
from customers as C join accounts as A
on C.CustomerID = A.CustomerID
group by C.CustomerID
order by total_balance desc
...
with highest_T as( 
select B.BranchName as branch, count(T.TransactionID) as no_of_transactions 
,dense_rank() over (order by count(T.TransactionID) desc) as top
...
select branch, no_of_transactions from highest_T
where top = 1;
